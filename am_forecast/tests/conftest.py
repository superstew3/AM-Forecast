...
def _reporting_settings_guard(request):
    """Put reporting_settings back however the run ends.

    Several fixtures move the reporting cut-off so a rule can be tested on a
    dataset that would not otherwise exercise it, and each restores it on the way
    out. That is correct until a run goes wrong: a fixture that fails after its
    UPDATE but before its yield never reaches its own restore, and the change is
    already committed. Nested save-and-restore fixtures make it worse, because an
    inner one can snapshot a value an outer one had already changed and faithfully
    restore the wrong thing.

    A real deployment was found with its cut-off left at 2025-12-31, set by
    pytest-admin, after a run with seventeen fixture errors. A wrong cut-off is
    not a cosmetic problem: it decides which months count as closed, so it moves
    every actual, every forecast and every budget in the system.

    This snapshots the row before anything runs and writes it back at the end,
    whatever happened in between. Per-test restores stay as they are — this is
    the backstop, not a replacement.
    """
    c = psycopg2.connect(request.config.getoption("--dsn"))
    try:
        with c.cursor() as cur:
            cur.execute("SELECT cut_off_date, cut_off_set_by, default_growth_pct "
            ...
            with c.cursor() as cur:
                cur.execute("""UPDATE reporting_settings
                               SET cut_off_date = %s, cut_off_set_by = %s,
                                   default_growth_pct = %s
                               WHERE id = 1
                                 AND (cut_off_date, cut_off_set_by, default_growth_pct)
                                     IS DISTINCT FROM (%s, %s, %s)""",
                            (*before, *before))
                if cur.rowcount:
                    print(f"\n[conftest] reporting_settings restored to {before[0]} "
                          f"— a fixture left it changed.")
            c.commit()
        except Exception:
            c.rollback()
    c.close()
